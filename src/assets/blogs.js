
import webtools from './GideonYegon.jpg';

const blogs=[
    {
        id:1,
        image:webtools,
        title:"10 Essential Web Development Tools for Beginners",
        description:"Web development can seem daunting to beginners, but the right tools can make the learning process smoother and more enjoyable. Here are ten essential web development tools that every beginner should consider. learn more: https://localhost:3000/blog/1"
    },
    {
        id:2,
        image:webtools,
        title:"10 Essential Web Development Tools for Beginners",
        description:"Web development can seem daunting to beginners, but the right tools can make the learning process smoother and more enjoyable. Here are ten essential web development tools that every beginner should consider: "
    }
]
export default blogs;